...
import { ChatConversation, ChatListView } from "./features/chat/ChatViews";
...
const MOCK_PROVIDERS: ProviderStore[] = [
  {
    ...
    description:
      "Especialista en instalaciones domiciliarias e industriales. Urgencias 24hs. Matriculado con más de 10 años de experiencia en CABA.",
    portfolioImages: [
  ...
  },
  {
    ...
    description:
      "Empresa líder en soluciones sanitarias. Destapaciones con máquina, filtraciones y reparaciones generales. Garantía escrita.",
    portfolioImages: [],
  },
  {
    id: "p3",
...
const MOCK_INCOMING_ORDERS: ServiceOrder[] = [
  {
    ...
    clientCoordinates: { x: -34.598, y: -58.386 },
  ...
  },
  {
    ...
    clientCoordinates: { x: -34.601, y: -58.383 },
...
const MOCK_CHATS: ChatSession[] = [
  {
    id: "c1",
    orderId: "req_101",
    participants: "Juan Pérez (Demo)",
    lastMessage: "Estoy llegando en 5 min.",
...
const App = () => {
  ...
  const [publicRequests, setPublicRequests] =
    useState<PublicRequest[]>(MOCK_PUBLIC_REQUESTS);

  const [selectedProvider, setSelectedProvider] =
    useState<ProviderStore | null>(null);
  ...
  const [viewingClientProfile, setViewingClientProfile] = useState<
    string | null
  >(null);
  ...
  const [savedAddresses, setSavedAddresses] =
    useState<SavedAddress[]>(MOCK_ADDRESSES);
  ...
  const [myProviderProfile, setMyProviderProfile] = useState<ProviderStore>(
    MOCK_PROVIDERS[0]
  );
  ...
  const handleAddAddress = (
    ...
    const baseLat = coordinates
      ? coordinates.x
      : !isNaN(mapCenter.lat)
      ? mapCenter.lat
      : DEFAULT_CENTER[0];
    const baseLng = coordinates
      ? coordinates.y
      : !isNaN(mapCenter.lng)
      ? mapCenter.lng
      : DEFAULT_CENTER[1];
  ...
  const handleSendRequest = async () => {
    ...
    setNotificationMessage(
      "Solicitud enviada. Se ha generado un chat con el profesional."
    );
  ...
  const handleAcceptOrder = (order: ServiceOrder) => {
    ...
    if (hasActiveJob) {
      setNotificationMessage(
        "Ya tienes un trabajo en curso. Termínalo antes de aceptar otro."
      );
      setIsNotificationModalOpen(true);
      return;
    }
          ...
          return {
            ...
            applicants: [
              ...r.applicants,
              {
                ...
                providerName: myProviderProfile.name,
                rating: myProviderProfile.rating,
                status: "PENDING",
              },
            ],
          };
  ...
  const handleAcceptApplicant = (
    ...
    setSelectedPublicRequest(null);
    setNotificationMessage(
      "Has contratado al profesional. Se ha abierto un chat."
    );
    setIsNotificationModalOpen(true);
  };
  ...
  const handleUpdateProfile = (updates: Partial<ProviderStore>) => {
    ...
    setProviders((prev) =>
      prev.map((p) =>
        p.id === myProviderProfile.id ? { ...p, ...updates } : p
      )
    );
  };
  ...
  if (
    ...
    return (
      <TrackingView
        ...
        providers={providers}
        onMinimize={() => setIsTrackingMaximized(false)}
    ...
    <div className="w-full h-screen bg-zinc-900 text-white font-sans overflow-hidden flex flex-col max-w-md mx-auto shadow-2xl border-x border-zinc-800">
      <div className="flex-1 relative overflow-hidden">
        ...
        {viewingClientProfile && (
          <ClientProfileSheet
            clientName={viewingClientProfile}
            onClose={() => setViewingClientProfile(null)}
          />
        )}
        {viewingStore && (
          <StorePreviewSheet
            provider={viewingStore}
        ...
        {activeChatSession && (
          <ChatConversation
            ...
            onViewProfile={() => {
              /* Todo: link to profile from chat */
            }}
          />
        )}
        ...
        <AddressModal
          ...
          currentAddressId={
            savedAddresses.find((a) => a.name === currentAddress)?.id ||
            (currentAddress === "Mi Ubicación Actual" ? "current" : undefined)
          }
        />
        <NotificationModal
