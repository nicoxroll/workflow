...
import { SavedAddress, ServiceCategory } from "../types";
...
export const Button: React.FC<ButtonProps> = ({
  ...
  const baseStyle =
    "w-full py-4 px-6 font-bold uppercase tracking-wider text-xs transition-all duration-150 active:translate-y-0.5 flex items-center justify-center gap-2 rounded-none border focus:outline-none";
...
export const Input: React.FC<InputProps> = ({
  ...
  <div className="flex flex-col gap-2 w-full">
    ...
    <div className="relative group">
      ...
      <input
        className={`w-full bg-black border border-zinc-800 text-white p-4 font-mono text-sm placeholder:text-zinc-700 focus:outline-none focus:border-white transition-colors rounded-none ${
          icon ? "pl-12" : ""
        } ${className}`}
        {...props}
      />
...
export const RequestItem: React.FC<{
  ...
  return (
    <div
      onClick={onClick}
      className={`group border-b border-zinc-900 py-5 hover:bg-zinc-900/50 cursor-pointer transition-colors px-4 ${
        isCompleted ? "opacity-60 grayscale" : ""
      }`}
    >
      <div className="flex justify-between items-start mb-1">
        <h3 className="font-bold text-white text-sm uppercase tracking-wide group-hover:underline decoration-1 underline-offset-4">
          {title}
        </h3>
        <span
  ...
  <div className="flex gap-0.5">
    {[1, 2, 3, 4, 5].map((star) => (
      <Star
        key={star}
        className={`w-3 h-3 ${
          star <= value ? "fill-white text-white" : "text-zinc-800"
        }`}
      />
    ))}
  </div>
    ...
    <div className="fixed inset-0 z-[2000] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      <div className="relative bg-black border-t sm:border border-white w-full max-w-md p-6 animate-in slide-in-from-bottom-10 shadow-2xl">
        ...
        <div className="space-y-8 mb-8">
          <div>
            <label className="text-[10px] uppercase font-bold text-zinc-500 tracking-widest mb-3 block">
              Rubro / Categoría
            </label>
          ...
          <div>
            ...
            <div className="flex gap-2">
              {[1, 2, 3, 4, 5].map((n) => (
                <button
                  key={n}
                  className="flex-1 border border-zinc-800 py-3 hover:bg-white hover:text-black hover:border-white transition-colors font-mono text-xs"
                >
                  {n}+
                </button>
              ))}
        ...
        {view === "list" ? (
          <>
            <div className="space-y-2 mb-6">
              <div
                ...
                } // Coordinates handled in parent
                className={`p-4 border cursor-pointer transition-colors flex items-center justify-between group ${
                  currentAddressId === "current"
                    ? "bg-white text-black border-white"
              ...
              {addresses.map((a) => {
                ...
                const icon = a.name.toLowerCase().includes("casa") ? (
                  <Home className="w-5 h-5" />
                ) : a.name.toLowerCase().includes("trabajo") ||
                  a.name.toLowerCase().includes("oficina") ? (
          ...
          <div className="animate-in slide-in-from-right">
            ...
            <h4 className="font-bold text-white mb-4">
              {view === "add" ? "Nueva Dirección" : "Editar Dirección"}
            </h4>
