...
export const OrdersView: React.FC<{
  ...
  const clientOrders = orders.filter((o) =>
    o.clientId.includes("Usuario Actual")
  );
    ...
    <div className="absolute inset-0 bg-black flex flex-col pt-6 pb-[100px]">
      <div className="px-6 mb-6 shrink-0">
        <h2 className="text-3xl font-black uppercase mb-1">
          {role === UserRole.PROVIDER ? "Mis Pedidos" : "Pedidos"}
        </h2>
        <p className="text-zinc-500 text-sm">
          {role === UserRole.PROVIDER
            ? "Solicitudes entrantes y activas"
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-4">
        ...
        {role === UserRole.CLIENT && (
          <>
            <div className="mb-6">
              <h3 className="text-xs font-bold uppercase text-zinc-500 tracking-widest mb-3 px-2">
                Mis Solicitudes Activas
              </h3>
              {publicRequests.filter((r) => r.ownerId === "current_user")
                .length === 0 ? (
                ...
                publicRequests
                  .filter((r) => r.ownerId === "current_user")
                  .map((req) => (
                    <div
                      key={req.id}
                      className="bg-zinc-900 p-4 border border-zinc-800 mb-2"
                    >
                      <div className="flex justify-between items-center mb-1">
                        <span className="font-bold text-white text-sm uppercase">
                          {req.category.name}
                        </span>
                        <span className="text-[10px] bg-blue-900 text-blue-200 px-2 py-0.5 rounded-full">
                          {req.applicants.length} Postulantes
                        </span>
                      </div>
                      <div className="text-xs text-zinc-500">
                        {req.offerPrice} • {req.description}
                      </div>
                    </div>
                  ))
              )}
            ...
            <div>
              ...
              {providers.map((prov) => (
                <div
                  ...
                  className="flex items-center gap-4 p-4 border-b border-zinc-900 hover:bg-zinc-900 cursor-pointer"
                >
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-xs text-zinc-500">
                      <span>{prov.categoryName}</span> •{" "}
                      <span className="flex items-center gap-0.5 text-white">
                        <Star className="w-2 h-2 fill-white" /> {prov.rating}
                      </span>
                    </div>
                  </div>
                </div>
